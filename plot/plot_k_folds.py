...
def plot_k_folds_time_series(model_results, model_names, time_series, k_fold_tr_idx_list, k_fold_val_idx_list, metric_um='', exog=None):
    ...
    for k in range(n_splits):
        ...
        plt.plot(pd.concat([data_tr_k, data_val_k], axis=0), marker='o', markersize=3, linewidth=2, label= 'true')
        ...
        plt.plot(model_result.fittedvalues, marker='o', markersize=2, linewidth=3,  label= f'{model_name} fitted')
        plt.plot(fcast.predicted_mean, line_pred_style, markersize=3, linewidth=2, label= f'{model_name} pred')
