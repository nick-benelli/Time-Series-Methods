...
def aggregate_weekly_data(
    ...
    data_weekly[datetime_name] = data_weekly[datetime_name] - pd.to_timedelta(
        7, unit="d"
    )

    grouper = data_weekly.groupby([pd.Grouper(key=datetime_name, freq="W-MON")])[
        metric_name
    ]
    ...
    try:
        ...
        data_weekly = (
            grouper.apply(method).reset_index(drop=True).sort_values(datetime_name)
        )
...
def aggregate_weekly_data_use_sum(
    data, metric_name, datetime_name="date", use_sum=True
) -> pd.Series:
    """
    Takes a time series daily values and aggregates them to weekly
    ---
